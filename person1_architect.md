...
**Reviewer:** SMU Cybersecurity Faculty  
...
| ID | Description |
| :--- | :--- |
| 1 | **Node.js Environment**: Hosted on SMU Halifax Campus production-grade Linux servers. |
...
| 4 | **SMU SMTP Relay**: Primary exit point for sending digital tickets and refund confirmations to user emails. |
...
| 1.4 | Refund Request | Endpoint for customers to request a `REFUNDED` state for their `PAID` tickets. | (2) |
| 1.5 | Staff Check-in | Mobile-optimized interface for on-site staff to mark tickets as `USED`. | (4) |
